function [result,U,Z,Theta]=CASO_MVMT(testFea,trainFea,trainLabel,unlabelFea,ite,h,alpha,beta,gamma)
...
for i=1:taskNum
    for j=1:viewNum
        tmp=cell2mat(testFea(i,j));
        tmpsize=size(tmp);
        if tmpsize(1)>0
            viewLabel(i,j)=1;
            featureNum(j)=tmpsize(2); 
            sampleNum(i)=tmpsize(1);
            testFea(i,j)=mat2cell(sparse(tmp),tmpsize(1),tmpsize(2));
        end
    end
end
...
for i=1:taskNum
    ...
    for j=tmpj
        tmpfea=cell2mat(testFea(i,j));
        tmpw=cell2mat(U(i,j));
        tmplabel=tmplabel+tmpfea*tmpw;
    end
    result{i} = tmplabel/length(tmpj);
end
% meanF1=mean(F1);
% meanError = mean(err);
